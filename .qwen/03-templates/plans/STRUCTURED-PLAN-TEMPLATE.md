# Structured Plan Template (Qwen-Optimized)
...
> - Use `@file:line` references for clickable navigation in supported IDEs
> - Reference SubAgents for specialized review tasks
> - Use Qwen's Plan Mode (`/plan`) for structured planning workflow
> - SubAgents can be spawned for parallel research using `general-purpose` type
...
> **Qwen Tip**: Use `@file:line` syntax for clickable references in Qwen Code.
...
> **Qwen Tip**: SubAgents can use `web_fetch` tool to retrieve these docs.
...
>
> **Qwen Tip**: Use `@file` references in PATTERN fields for IDE navigation. SubAgents can be
> spawned for specialized tasks using their agent names (e.g., `/subagent code-review-security`).
>
...
> **Qwen Tip**: These commands can be run directly in Qwen Code's terminal or via Bash tool.
