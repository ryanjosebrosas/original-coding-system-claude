...
**If plan-validator agent exists** in `.qwen/agents/`: Use the @plan-validator agent to validate the plan structure. Review findings. If Critical issues found, report to user before proceeding. If no critical issues, continue.

**If agent not available**: Skip — proceed to execution.
...
**d.** Mark task as complete in your tracking.
...
2. Execute tasks using Step 2 process
...
**IMPORTANT**: Save the report to the file FIRST, then also display it inline for the user.
