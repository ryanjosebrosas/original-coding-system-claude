...
# Code Review Fix: Implement Review Findings
...
Before fixing each issue, evaluate whether it SHOULD be fixed. May choose NOT to fix issues that are: documented as acceptable, out of scope, would require architectural changes, style preferences not bugs, or false positives. **Explain every skip.** Default bias: fix it.
