...
**If custom review agents exist** in `.qwen/agents/`: Use **Parallel Review** (4 agents, 40-50% faster)
**If not**: Use **Standard Review** (single-agent sequential)

```bash
ls .qwen/agents/code-review-*.md 2>/dev/null | wc -l
```
...
Read @QWEN.md and project README for standards.
...
Check for: logic errors, security issues, performance problems, code quality (DRY, naming, types), standards compliance (QWEN.md conventions, linting, testing patterns).
