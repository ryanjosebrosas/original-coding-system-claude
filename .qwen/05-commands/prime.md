...
- **prompt**: |
    You are analyzing a Qwen Code project's available commands.
    Run: `grep -r "^description:" .qwen/commands/ 2>/dev/null` via Bash to get all command descriptions.
    Also check for skills: `ls .qwen/skills/ 2>/dev/null` via Bash.
...
- **prompt**: |
    You are analyzing a Qwen Code project's available agents.
    Read the file `.qwen/agents/README.md` for the complete agent catalog.
...
- **prompt**: |
    ...
    - **Tech Stack**: {languages, frameworks, tools — e.g., "Markdown, Qwen Code CLI, Git"}
...
- **prompt**: |
    ...
    Skip .qwen/ directory contents — those are tooling, not the app.
